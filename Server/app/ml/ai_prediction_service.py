...
--------------------------
Vendored Model 2 (Dynamic Rainfall-Triggered Landslide Risk) implementing the
GET /ai/prediction contract (gauge + factors + 24h trend).

This is the trained-model layer: XGBoost probability + transparent rule-based
composite, fused via `alpha`, with SHAP-derived per-factor weights. It is kept
dependency-isolated (numpy/pandas/xgboost/shap/joblib) and is imported lazily by
app.services.prediction_service so the rest of the API boots even if these heavy
ML deps or the model artifact are unavailable.

Public API:
    get_ai_prediction(zones, zone_id=None, district=None) -> dict
    score_snapshot(features) -> (xgb_probability, composite_score, fused_score)

Model artifact resolution (robust, no dependence on CWD):
    1. MODEL2_PATH environment variable, if set
    2. model2_weighted.joblib next to this file (default)
"""
...
# Robust model path: env override, else the artifact shipped beside this module.
MODEL_PATH = os.environ.get("MODEL2_PATH") or str(
    Path(__file__).resolve().with_name("model2_weighted.joblib")
)
...
def _factor_levels(features):
    ...
    # Guard against None (unavailable features) so bucketing never crashes.
    rain_15d = rain_15d if rain_15d is not None else 0
    soil_moisture = soil_moisture if soil_moisture is not None else 0
    susceptibility = susceptibility if susceptibility is not None else 0
    disturbance = disturbance if disturbance is not None else 0
    ...
    slope_val = features.get("slope_angle_deg")
    slope_val = slope_val if slope_val is not None else 0
...
def get_ai_prediction(zones, zone_id=None, district=None):
    ...
    return {
        ...
        "factors": factors,
        "trend": trend,
        "trend_source": trend_source,  # extra field: "real" or "simulated" -- data-quality signal
    }
